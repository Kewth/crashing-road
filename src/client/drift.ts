...
function update_drift(car: Car) {
    if(is_drifting(car)) {
        let carHeading = getHeadingDirection(car)
        ...
        drift_mark3.addin(car.scene, car.world)
        drift_mark3.update()
        ...
        drift_mark4.addin(car.scene, car.world)
        drift_mark4.update()
    }
}
...
function getHeadingDirection(car: Car): THREE.Vector3 {
    // let euler = new THREE.Euler();
    // euler.setFromQuaternion(car.obj3d.quaternion);

    // // Convert the Euler angles to a direction vector
    // let headingDirection = new THREE.Vector3();
    // headingDirection.setFromSphericalCoords(1, euler.y, euler.x);

    const headingDirection = car.wheelBodys[0].position.vsub(car.wheelBodys[2].position)

    const res = new THREE.Vector3(headingDirection.x, headingDirection.y, 0)
...
function is_drifting(car: Car) {
    ...
    const car_heading = getHeadingDirection(car)
